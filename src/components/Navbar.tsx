...
export const Navbar: React.FC<NavbarProps> = ({ activeSection, onNavigate }) => {
  ...
  return (
    <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 px-4 sm:px-8 ${
      isScrolled 
        ? 'bg-[#0A0A0C]/90 backdrop-blur-xl border-b border-white/10 shadow-2xl py-3 sm:py-3.5' 
        : 'bg-gradient-to-b from-[#0A0A0C]/90 via-[#0A0A0C]/40 to-transparent backdrop-blur-[2px] py-4 sm:py-6 border-b border-transparent'
    }`}>
      <div className="max-w-7xl mx-auto flex items-center justify-between">
        ...
        <div className="flex md:hidden items-center gap-2">
          ...
          <div className="flex items-center p-0.5 rounded-full glass-card border border-white/15 text-[11px] font-mono">
            <button
              onClick={() => setLang('EN')}
              className={`px-2 py-0.5 rounded-full ${lang === 'EN' ? 'bg-white text-black font-bold' : 'text-neutral-400'}`}
            >
              EN
            </button>
            <button
              onClick={() => setLang('AL')}
              className={`px-2 py-0.5 rounded-full ${lang === 'AL' ? 'bg-white text-black font-bold' : 'text-neutral-400'}`}
            >
              AL
            </button>
          </div>
          ...
          <button
            ...
            className="w-10 h-10 rounded-full glass-nav flex items-center justify-center text-white focus:outline-none"
      ...
      {/* Mobile Drawer Menu */}
      {mobileMenuOpen && (
        <div className="md:hidden fixed inset-x-4 top-20 p-6 rounded-3xl glass-nav border border-white/15 shadow-2xl backdrop-blur-2xl flex flex-col gap-4 pointer-events-auto animate-in fade-in slide-in-from-top-4 duration-300">
          ...
          <div className="flex items-center justify-between pb-3 border-b border-white/10">
            ...
            <div className="flex items-center p-1 rounded-full glass-card border border-white/15 text-xs font-mono">
              <button
                onClick={() => setLang('EN')}
                className={`px-3 py-1 rounded-full ${lang === 'EN' ? 'bg-white text-black font-extrabold' : 'text-neutral-400'}`}
              >
                ENGLISH (EN)
              </button>
              <button
                onClick={() => setLang('AL')}
                className={`px-3 py-1 rounded-full ${lang === 'AL' ? 'bg-white text-black font-extrabold' : 'text-neutral-400'}`}
              >
                SHQIP (AL)
              </button>
            </div>
          ...
          <div className="flex flex-col gap-1 pb-4 border-b border-white/10">
            {navLinks.map((link) => (
              <button
                ...
                className={`flex items-center justify-between px-4 py-3 rounded-2xl text-sm font-medium tracking-wide transition-all ${
                  activeSection === link.href
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-2 py-3.5 rounded-2xl bg-white text-black font-semibold text-xs tracking-wider uppercase shadow-xl"
          >
            <span>{t.nav.startProject}</span>
